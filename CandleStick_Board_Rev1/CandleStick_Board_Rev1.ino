...
{
  if (percent > 100)
    percent = 100;    // sanitize the input
  ...
  if (tempTimerExpired())
  {
    ...
    if (isSetpointTimerExpired == EXPIRED || isSetpointTimerExpired == HOLD)
    {
      ...
      if (phaseShiftOutput != ON)
      {
        ...
        if (prevPhaseShiftOutput == ARMED)
          prevPhaseShiftOutput = OFF;
        phaseShiftOutput = DISABLED;
...
void phaseShiftOutputControl(void)
{
  ...
  if (okToRunSlowSoftware || phaseShiftOutput == DISABLED)
    nonEssentialCode();
  okToRunSlowSoftware = false;
}
...
void increaseDecreaseLineSelected(int direction)
{
  if (!updateNumberMode)
  {
    ...
    if (lineSelected > getNumberOfOptions() - 1)
    {
      lineSelected = 0;
    };
    if (lineSelected < 0)
    {
      lineSelected = getNumberOfOptions() - 1;
    };
    screenManager(home_stringTable, 0);
  }
}
...
void increaseDecreaseTempSetpoint(int direction)
{
  if (direction == up)
  {
    storedSetpoint = storedSetpoint + setpointStepSize;
  }
  if (direction == down)
  {
    storedSetpoint = storedSetpoint - setpointStepSize;
  ...
  if (storedSetpoint < 0)
    storedSetpoint = 0;
  if (storedSetpoint > upperAllowableTemperature)
    storedSetpoint = upperAllowableTemperature;
